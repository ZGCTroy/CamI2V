...
from demo.realcami2v_test import Image2Video
...
def load_example():
    ...
    data = []
    for path in args.example_meta_path:
        with open(path, "r") as f:
            data.extend([[example[x] for x in key_order] for example in json.load(f)])
...
def get_tab(image2video: Image2Video, resolution: tuple[int, int], captioner: Qwen2VL_Captioner = None):
    ...
    with gr.Tab("x".join(map(str, resolution))) as tab:
        ...
        with gr.Row():
            preview_video = gr.Video(label="Preview Video", elem_id="output_vid", interactive=False, autoplay=True, loop=True)
        ...
        with gr.Row():
            with gr.Column():
                ...
                trace_extract_ratio = gr.Slider(label="Trace Extract Ratio", minimum=0, maximum=1.0, step=0.1, value=1.0)
                trace_scale_factor = gr.Slider(label="Camera Trace Scale Factor", minimum=0, maximum=5, step=0.1, value=0.5)

            with gr.Column():
                noise_shaping = gr.Checkbox(label='Enable Noise Shaping', value=False)
                noise_shaping_minimum_timesteps = gr.Slider(label="Noise Shaping Minimum Timesteps", minimum=0, maximum=999, step=1, value=900)
        ...
        with gr.Row():
            ...
            generate_btn.click(
                fn=generate,
                inputs=[model_name, input_image, input_text, negative_prompt, camera_pose_type, trace_extract_ratio, frame_stride, steps, trace_scale_factor, camera_cfg, cfg_scale, seed, enable_camera_condition, noise_shaping, noise_shaping_minimum_timesteps],
                outputs=[output_video1, output_3d, preview_video],
            )
        ...
        gr_examples = gr.Examples(
            ...
            inputs=[input_image, input_text, noise_shaping, noise_shaping_minimum_timesteps],
            examples_per_page=-1,
        )
...
def get_demo(args):
    ...
    """) as demo:
        gr.Markdown("""
            <div align='center'>
                <h1> RealCam-I2V: Real-World Image-to-Video Generation with Interactive Complex Camera Control </h1>
            </div>
        """)
...
def get_parser():
    ...
    parser.add_argument("--example_meta_path", type=str, nargs="+", default="./demo/examples.json")
